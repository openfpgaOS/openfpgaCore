//
// AXI4 Slave for SRAM (single-word async only).
//
// SRAM has no burst support — all reads and writes are single-word.
// Externally identical to the CRAM1 slave, but instantiated separately so
// the per-target slaves stay clean and independently testable.
//
...
// 1-entry R skid (same rationale as axi_sdram_slave / axi_cram1_slave)
reg [31:0] rskid_data;
reg        rskid_last;
reg        rskid_valid;
...
always @(posedge clk or posedge reset) begin
    if (reset) begin
        ...
        // Defaults.  rvalid / bvalid are handled by the hold-until-ready
        // block below.
        s_axi_arready <= 0;
        s_axi_awready <= 0;
        s_axi_wready <= 0;
