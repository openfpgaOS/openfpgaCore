//
// AXI4 SDRAM Arbiter — 3 Masters, Fixed Priority (v2).
//
...
//   M2 = Audio  (audio_dma, read-only)
//
// Fixed priority: GPU > CPU > Audio.
//
...
module axi_sdram_arbiter #(
    ...
    // Master 2: Audio DMA (read-only, lowest priority)
...
reg [1:0] arb_state;
reg [1:0] grant;  // 0=GPU, 1=CPU, 2=Audio
...
// Upstream rready — mux from granted master.  GPU (m0) and audio (m2)
// don't back-pressure so they default to 1; CPU (m1) drives m1_rready
// via cpu_target_port's registered response slot.
