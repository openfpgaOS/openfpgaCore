...
#   I-cache: 32KB (512 sets x 1 way x 64B line, direct-mapped)
#   D-cache: 128KB (1024 sets x 2 ways x 64B line, write-back, 2-way set-associative)
...
sbt "runMain vexiiriscv.Generate \
      ...
      --fetch-l1-hardware-prefetch=nl --fetch-wishbone \
      ...
      --relaxed-src \
      --reset-vector=0 \
      --region base=0,size=40000,main=1,exe=1 \
...
# Memory regions:
#   0x00000000  64KB  BRAM region    (cached, executable; 192KB physical)
