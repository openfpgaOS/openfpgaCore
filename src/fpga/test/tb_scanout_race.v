//------------------------------------------------------------------------------
// SPDX-License-Identifier: Apache-2.0
// SPDX-FileType: SOURCE
// SPDX-FileCopyrightText: (c) 2026, ThinkElastic <Think@Elastic.com>
//------------------------------------------------------------------------------
//
// Verilator integration testbench: SCANOUT-vs-GPU read-during-write RACE across
// the triple-buffer page flip on the MiSTer target.  Wires the REAL data path:
//
//   video_CRT_scanout_indexed_BRAM (LCD line fetch, burst_rd master)
//        -> io_sdram burst_rd port (hard-real-time, OUTRANKS word ops in ST_IDLE)
//        \
//         +--> sdram_model_full (shared SDRAM holding all three FBs)
//        /
//   gpu_core (m_wr_* FB writes via its own fbwq coalescer)
//        -> axi_sdram_arbiter M0 -> axi_sdram_slave -> io_sdram word ports
//
// plus the triple-buffer SWAP STATE MACHINE, an EXACT test-side copy of the
// production logic in axi_periph_slave.v:
//   * gpu_swap_req side-port latch        (axi_periph_slave.v:1506-1509)
//   * vsync / early_vblank consume         (axi_periph_slave.v:1489-1500)
//   * fb_display_idx -> fb_display_addr mux (axi_periph_slave.v:758,785-791)
//   * fb_swap_pending_o feedback -> gpu_core.slave_swap_pending (the REAL
//     CMD_FLIP backpressure path, gpu_core.v:4322)
// The swap SM is modelled in this TB (not the full axi_periph_slave) so the
// bench stays focused on the flip/scanout interaction; it is byte-faithful to
// the cited production lines and is clearly TEST-ONLY.
//
// The scanout reads fb_display_addr from the SHARED SDRAM, so it returns
// whatever the GPU actually wrote (or the clear color, if the GPU has not yet
// redrawn that word).  The harness compares the scanned-out pixels against the
// golden committed framebuffer and flags any pixel that scans out as clear/black
// where the golden frame has content == the read-during-write defect.
//
// Three clock domains, all derived from `clk` (= clk_sdram = clk_ram_controller,
// 100 MHz on MiSTer):
//   clk_sdram  = clk           (100 MHz)  — arbiter/slave/io_sdram/gpu_core/scanout SDRAM FSM
//   clk_vid    = clk / CLKDIV  (24.576 MHz nominal; CLKDIV models the 100:24.576
//                               ratio coarsely — the test cares about the *ratio*
//                               of scanout fetch rate to GPU write rate, not exact
//                               sub-ns timing) — drives x_count/y_count raster
//   clk_analog = clk_vid                  — MiSTer wires clk_analog = clk_vid
//
// TEST-ONLY, ADDITIVE.  No production RTL is modified.
//
`default_nettype none

module tb_scanout_race #(
    parameter GPU_HAS_VERT_TRI       = 1,
    parameter GPU_HAS_PARAM_TRI_RECS = 1,
    parameter GPU_Z_READ_WINDOW      = 4,
    parameter GPU_EW_PARALLEL_DIVS   = 1,
    parameter GPU_HAS_COMPACT_SPAN   = 1,
    parameter GPU_HAS_COLUMN_LIST    = 1,
    parameter BANK_ROW_TRACK         = 1,
    // clk:clk_vid divider.  16 => clk_vid = 100/16 = 6.25 MHz model rate; the
    // RATIO that matters is (clk_sdram cycles per scanout line-burst) — see the
    // C++ notes.  Overridable from the Makefile to sweep GPU-vs-scanout rate.
    parameter CLKDIV                 = 4,
    // out_height: MiSTer pins this to 480 for EVERY mode (emu.sv:990 — the
    // 2026-06-12 ascal-blank fix).  Pocket uses the app-native height
    // (scaler_slot_height, e.g. 200/240).  Drive 480 to model MiSTer, the
    // native height to model Pocket.
    parameter OUT_HEIGHT             = 480
) (
    input  wire        clk,            // 100 MHz: clk_sdram == clk_ram_controller
    input  wire        reset_n,

    // ---- GPU MMIO (driven by C++ harness, mirrors tb_gpu_wrpath) ----
    input  wire        reg_wr,
    input  wire [3:0]  reg_addr,
    input  wire [31:0] reg_wdata,
    output wire [31:0] reg_rdata,
    output wire        busy,
    output wire [31:0] fence_reached,
    output wire [5:0]  dbg_state,

    // ---- CPU FB_SWAP_CTRL write path (axi_periph_slave.v:1317-1320) ----
    // Models of_video_flip()/the acquire_next !fence_ok fallback, which write
    // FB_SWAP_CTRL directly and DO NOT go through gpu_core's slave_swap_pending
    // backpressure.  When fb_swap_ctrl_wr pulses, {ready_idx<=wdata[2:1];
    // swap_pending<=1} exactly as the sysreg write.
    input  wire        fb_swap_ctrl_wr,
    input  wire [31:0] fb_swap_ctrl_wdata,

    // ---- Scanout / raster config (from C++) ----
    input  wire [24:0] fb_addr_0,      // word base of FB 0 in SDRAM (>>1 byte)
    input  wire [24:0] fb_addr_1,
    input  wire [24:0] fb_addr_2,
    input  wire [9:0]  fb_w,           // source FB width  (pixels)
    input  wire [9:0]  fb_h,           // source FB height (lines)
    input  wire [15:0] fb_strideb,     // source FB stride (bytes)
    input  wire [9:0]  out_w,          // raster active width  (640)
    input  wire [2:0]  color_mode_in,  // OF_VIDEO_MODE_* (0 = 8bit indexed)

    // ---- SDRAM backdoor (preload ring/tex/FB; verify FB + scanout reads) ----
    input  wire        bd_we,
    input  wire [23:0] bd_addr,        // word address
    input  wire [31:0] bd_wdata,
    input  wire [23:0] bd_rd_addr,     // word address
    output wire [31:0] bd_rd_data,

    // ---- Swap SM observation/inspection (for the detector) ----
    output wire [1:0]  fb_display_idx_o,
    output wire [1:0]  fb_ready_idx_o,
    output wire        fb_swap_pending_o,
    output wire [24:0] scanout_burst_addr_o,
    output wire        scanout_burst_rd_o,
    output wire        scanout_burst_data_valid_o,
    output wire [31:0] scanout_burst_data_o,
    output wire        gpu_swap_req_o,
    output wire [1:0]  gpu_swap_idx_o,
    output wire [9:0]  y_count_o,
    output wire [9:0]  x_count_o,
    // FB_SWAP_CTRL read shape (regs.h:119 / axi_periph_slave.v:1634):
    // bit0 = pending, bits[2:1] = current display idx.  The firmware's
    // sync_swap_state reads exactly this.
    output wire [31:0] fb_swap_ctrl_rd_o,

    // ---- raster event strobes (1-cycle in clk_sdram domain) ----
    output wire        vsync_rising_o,
    output wire        frame_start_o
);

// ============================================================
// Clock generation: clk_vid = clk / CLKDIV (rising-edge gen in clk domain).
// ============================================================
reg [7:0] clkdiv_cnt;
reg       clk_vid;
always @(posedge clk or negedge reset_n) begin
    if (!reset_n) begin
        clkdiv_cnt <= 8'd0;
        clk_vid    <= 1'b0;
    end else begin
        if (clkdiv_cnt >= (CLKDIV[7:0] - 8'd1)) begin
            clkdiv_cnt <= 8'd0;
            clk_vid    <= ~clk_vid;
        end else begin
            clkdiv_cnt <= clkdiv_cnt + 8'd1;
        end
    end
end
// analog_ce_pix: MiSTer's clk_analog == clk_vid, analog_ce_pix is a 1-in-2 of
// clk_vid in the real wiring (12.288 strobe off the 24.576 clk).  Model it as a
// toggle in the clk_vid domain.
reg analog_ce_pix;
always @(posedge clk_vid or negedge reset_n)
    if (!reset_n) analog_ce_pix <= 1'b0;
    else          analog_ce_pix <= ~analog_ce_pix;

// ============================================================
// MiSTer raster (emu.sv:927-1108): 640x480 @ ~60 Hz, V_TOTAL 525, H_TOTAL 780.
// ============================================================
localparam [9:0] CRT_H_SYNC   = 10'd58;
localparam [9:0] CRT_H_BPORCH = 10'd62;
localparam [9:0] CRT_H_FPORCH = 10'd20;
localparam [9:0] CRT_H_TOTAL  = CRT_H_SYNC + CRT_H_BPORCH + 10'd640 + CRT_H_FPORCH; // 780
localparam [9:0] CRT_V_SYNC   = 10'd3;
localparam [9:0] CRT_V_BPORCH = 10'd15;
localparam [9:0] CRT_V_ACTIVE_BASE = CRT_V_SYNC + CRT_V_BPORCH; // 18
localparam [9:0] CRT_V_TOTAL  = 10'd525;
wire [9:0] crt_v_active = OUT_HEIGHT[9:0];
wire [9:0] crt_v_center_offset =
    (crt_v_active < 10'd240) ? ((10'd240 - crt_v_active) >> 1) : 10'd0;
wire [9:0] crt_v_active_start = CRT_V_ACTIVE_BASE + crt_v_center_offset;

reg [9:0] x_count, y_count;
reg       line_start;
reg       vsync_r;       // active high during V sync lines (matches emu crt_vs)
always @(posedge clk_vid or negedge reset_n) begin
    if (!reset_n) begin
        x_count <= 10'd0; y_count <= 10'd0; line_start <= 1'b0; vsync_r <= 1'b0;
    end else begin
        line_start <= (x_count == 10'd0);
        x_count <= x_count + 10'd1;
        if (x_count == CRT_H_TOTAL - 10'd1) begin
            x_count <= 10'd0;
            y_count <= (y_count >= CRT_V_TOTAL - 10'd1) ? 10'd0 : y_count + 10'd1;
        end
        vsync_r <= (y_count < CRT_V_SYNC);
    end
end
assign y_count_o = y_count;
assign x_count_o = x_count;

// early_vblank: high before the visible active region starts (matches emu.sv
// early_vblank_safe_vid = y_count < crt_v_active_start).
wire early_vblank_vid = (y_count < crt_v_active_start);

// ============================================================
// vsync / early_vblank CDC into clk_sdram + edge detect (axi_periph_slave.v
// 834: vsync_rising = vsync_sync[1] && !vsync_sync[2]; 836-840 early_vblank_s).
// ============================================================
reg [2:0] vsync_sync;
reg [2:0] early_vblank_sync;
always @(posedge clk or negedge reset_n) begin
    if (!reset_n) begin
        vsync_sync <= 3'd0; early_vblank_sync <= 3'd0;
    end else begin
        vsync_sync        <= {vsync_sync[1:0], vsync_r};
        early_vblank_sync <= {early_vblank_sync[1:0], early_vblank_vid};
    end
end
wire vsync_rising = vsync_sync[1] && !vsync_sync[2];
wire early_vblank_s = early_vblank_sync[2];
assign vsync_rising_o = vsync_rising;
assign frame_start_o  = vsync_rising;

// ============================================================
// SWAP STATE MACHINE — exact copy of axi_periph_slave.v:758-762, 785-791,
// 1489-1509.  os_inmenu tied 0 (app owns display).  Driven by the real
// gpu_swap_req from gpu_core.  All in clk_sdram domain like production.
// ============================================================
reg [1:0] fb_display_idx;
reg [1:0] fb_ready_idx;
reg       fb_swap_pending;
reg       fb_swap_consumed_this_frame;
assign fb_swap_pending_o = fb_swap_pending;
assign fb_display_idx_o  = fb_display_idx;
assign fb_ready_idx_o    = fb_ready_idx;

wire        gpu_swap_req;
wire [1:0]  gpu_swap_idx;
assign gpu_swap_req_o = gpu_swap_req;
assign gpu_swap_idx_o = gpu_swap_idx;

always @(posedge clk or negedge reset_n) begin
    if (!reset_n) begin
        fb_display_idx              <= 2'd0;
        fb_ready_idx                <= 2'd0;
        fb_swap_pending             <= 1'b0;
        fb_swap_consumed_this_frame <= 1'b0;
    end else begin
        // axi_periph_slave.v:1489-1500 (os_inmenu = 0)
        if (vsync_rising)
            fb_swap_consumed_this_frame <= 1'b0;
        if (fb_swap_pending && vsync_rising) begin
            fb_display_idx <= fb_ready_idx;
            fb_swap_pending <= 1'b0;
            fb_swap_consumed_this_frame <= 1'b1;
        end else if (fb_swap_pending && early_vblank_s &&
                     !fb_swap_consumed_this_frame) begin
            fb_display_idx <= fb_ready_idx;
            fb_swap_pending <= 1'b0;
            fb_swap_consumed_this_frame <= 1'b1;
        end
        // CPU FB_SWAP_CTRL write (axi_periph_slave.v:1317-1320, sysreg 6'd6):
        // if (req_wdata[0]) { fb_ready_idx<=req_wdata[2:1]; swap_pending<=1; }.
        // This is the of_video_flip() / fallback path with NO gpu_core gating.
        if (fb_swap_ctrl_wr && fb_swap_ctrl_wdata[0]) begin
            fb_ready_idx    <= fb_swap_ctrl_wdata[2:1];
            fb_swap_pending <= 1'b1;
        end

        // axi_periph_slave.v:1506-1509 — CMD_FLIP side-port, placed LAST so its
        // assignments win same-cycle conflicts (later non-blocking wins).
        if (gpu_swap_req) begin
            fb_ready_idx    <= gpu_swap_idx;
            fb_swap_pending <= 1'b1;
        end
    end
end

// fb_display_addr mux (axi_periph_slave.v:785-788, term_fb_active = 0).
wire [24:0] fb_display_addr =
    (fb_display_idx == 2'd0) ? fb_addr_0 :
    (fb_display_idx == 2'd1) ? fb_addr_1 : fb_addr_2;

// FB_SWAP_CTRL read mux (axi_periph_slave.v:1634, sysreg 6'd6):
// {29'b0, fb_display_idx, fb_swap_pending}.
assign fb_swap_ctrl_rd_o = {29'b0, fb_display_idx, fb_swap_pending};

// ============================================================
// gpu_core master channels
// ============================================================
wire        gpu_rd_arvalid, gpu_rd_arready;
wire [31:0] gpu_rd_araddr;
wire [7:0]  gpu_rd_arlen;
wire        gpu_rd_rvalid;
wire [31:0] gpu_rd_rdata;
wire        gpu_rd_rlast;

wire        gpu_wr_awvalid, gpu_wr_awready;
wire [31:0] gpu_wr_awaddr;
wire [7:0]  gpu_wr_awlen;
wire        gpu_wr_wvalid, gpu_wr_wready;
wire [31:0] gpu_wr_wdata;
wire [3:0]  gpu_wr_wstrb;
wire        gpu_wr_wlast;
wire        gpu_wr_bvalid;

wire        gpu_sram_rd, gpu_sram_wr, gpu_sram_rd_half, gpu_sram_rd_hi;
wire [21:0] gpu_sram_addr;
wire [31:0] gpu_sram_wdata;
wire [3:0]  gpu_sram_wstrb;
reg  [31:0] gpu_sram_rdata;
reg         gpu_sram_busy;
reg         gpu_sram_rdata_valid;

gpu_core #(
    .GPU_HAS_VERT_TRI(GPU_HAS_VERT_TRI),
    .GPU_HAS_PARAM_TRI_RECS(GPU_HAS_PARAM_TRI_RECS),
    .GPU_Z_READ_WINDOW(GPU_Z_READ_WINDOW),
    .GPU_EW_PARALLEL_DIVS(GPU_EW_PARALLEL_DIVS),
    .GPU_HAS_COMPACT_SPAN(GPU_HAS_COMPACT_SPAN),
    .GPU_HAS_COLUMN_LIST(GPU_HAS_COLUMN_LIST)
) gpu (
    .clk(clk), .reset_n(reset_n), .gpu_enable(1'b1),
    .m_rd_arvalid(gpu_rd_arvalid), .m_rd_arready(gpu_rd_arready),
    .m_rd_araddr(gpu_rd_araddr),   .m_rd_arlen(gpu_rd_arlen),
    .m_rd_rvalid(gpu_rd_rvalid),   .m_rd_rdata(gpu_rd_rdata), .m_rd_rlast(gpu_rd_rlast),
    .m_wr_awvalid(gpu_wr_awvalid), .m_wr_awready(gpu_wr_awready),
    .m_wr_awaddr(gpu_wr_awaddr),   .m_wr_awlen(gpu_wr_awlen),
    .m_wr_wvalid(gpu_wr_wvalid),   .m_wr_wready(gpu_wr_wready),
    .m_wr_wdata(gpu_wr_wdata),     .m_wr_wstrb(gpu_wr_wstrb), .m_wr_wlast(gpu_wr_wlast),
    .m_wr_bvalid(gpu_wr_bvalid),
    .sram_rd(gpu_sram_rd), .sram_wr(gpu_sram_wr),
    .sram_rd_half(gpu_sram_rd_half), .sram_rd_hi(gpu_sram_rd_hi),
    .sram_addr(gpu_sram_addr), .sram_wdata(gpu_sram_wdata), .sram_wstrb(gpu_sram_wstrb),
    .sram_rdata(gpu_sram_rdata), .sram_busy(gpu_sram_busy), .sram_rdata_valid(gpu_sram_rdata_valid),
    .gpu_swap_req(gpu_swap_req), .gpu_swap_idx(gpu_swap_idx),
    // THE REAL backpressure path: gpu_core stalls CMD_FLIP while the slave's
    // pending swap slot is full (gpu_core.v:4322).
    .slave_swap_pending(fb_swap_pending),
    .reg_wr(reg_wr), .reg_addr(reg_addr), .reg_wdata(reg_wdata), .reg_rdata(reg_rdata),
    .busy(busy), .fence_reached(fence_reached),
    .dbg_state(dbg_state), .dbg_setup_step(), .dbg_aux(), .dbg_frag()
);

// SRAM scratch model (1-cycle M10K stub, same as tb_gpu_wrpath).
reg [31:0] sram_mem [0:65535];
always @(posedge clk or negedge reset_n) begin
    if (!reset_n) begin
        gpu_sram_busy <= 0; gpu_sram_rdata_valid <= 0; gpu_sram_rdata <= 0;
    end else begin
        gpu_sram_rdata_valid <= 0;
        gpu_sram_busy <= 0;
        if (gpu_sram_wr) begin
            if (gpu_sram_wstrb[0]) sram_mem[gpu_sram_addr[15:0]][7:0]   <= gpu_sram_wdata[7:0];
            if (gpu_sram_wstrb[1]) sram_mem[gpu_sram_addr[15:0]][15:8]  <= gpu_sram_wdata[15:8];
            if (gpu_sram_wstrb[2]) sram_mem[gpu_sram_addr[15:0]][23:16] <= gpu_sram_wdata[23:16];
            if (gpu_sram_wstrb[3]) sram_mem[gpu_sram_addr[15:0]][31:24] <= gpu_sram_wdata[31:24];
        end
        if (gpu_sram_rd) begin
            gpu_sram_rdata <= sram_mem[gpu_sram_addr[15:0]];
            gpu_sram_rdata_valid <= 1;
        end
    end
end

// ============================================================
// Arbiter <-> Slave wires
// ============================================================
wire        arb_s_arvalid, arb_s_arready;
wire [31:0] arb_s_araddr;
wire [7:0]  arb_s_arlen;
wire        arb_s_rvalid,  arb_s_rready;
wire [31:0] arb_s_rdata;
wire [1:0]  arb_s_rresp;
wire        arb_s_rlast;
wire        arb_s_awvalid, arb_s_awready;
wire [31:0] arb_s_awaddr;
wire [7:0]  arb_s_awlen;
wire        arb_s_wvalid,  arb_s_wready;
wire [31:0] arb_s_wdata;
wire [3:0]  arb_s_wstrb;
wire        arb_s_wlast;
wire        arb_s_bvalid;
wire [1:0]  arb_s_bresp;
wire        arb_s_wcont;

// Slave <-> io_sdram word interface
wire        sdram_slave_rd, sdram_slave_wr;
wire [23:0] sdram_slave_addr;
wire [31:0] sdram_slave_wdata;
wire [3:0]  sdram_slave_wstrb;
wire [3:0]  sdram_slave_burst_len;
wire [3:0]  sdram_slave_burst_wr_len;
wire        sdram_slave_wr_data_next;
wire        sdram_slave_wr_done;
wire [31:0] sdram_slave_next_wdata;
wire [3:0]  sdram_slave_next_wstrb;
wire [31:0] sdram_slave_preload_wdata;
wire [3:0]  sdram_slave_preload_wstrb;

reg         ram1_word_rd, ram1_word_wr;
reg  [23:0] ram1_word_addr;
reg  [31:0] ram1_word_data;
reg  [3:0]  ram1_word_wstrb;
reg  [31:0] ram1_word_data_next;
reg  [3:0]  ram1_word_wstrb_next;
reg  [3:0]  ram1_word_burst_len;
reg  [3:0]  ram1_word_burst_wr_len;
wire [31:0] ram1_word_q;
wire        ram1_word_busy;
wire        ram1_word_q_valid;

wire        phy_cke, phy_clk, phy_cas, phy_ras, phy_we;
wire [1:0]  phy_ba;
wire [12:0] phy_a;
wire [1:0]  phy_dqm;
wire [15:0] ctrl_dq_out;
wire        ctrl_dq_oe;
wire [15:0] model_dq_out;
wire        model_dq_oe;
wire [15:0] dq_to_ctrl = model_dq_oe ? model_dq_out : 16'h0;

// ============================================================
// Slave -> io_sdram pulse adapter (byte-identical to core_top.v / tb_gpu_wrpath)
// ============================================================
reg sdram_accepted_r;
reg sdram_cmd_forwarded;
always @(posedge clk or negedge reset_n) begin
    if (!reset_n) begin
        ram1_word_rd <= 0; ram1_word_wr <= 0;
        ram1_word_addr <= 0; ram1_word_data <= 0; ram1_word_wstrb <= 0;
        ram1_word_data_next <= 0; ram1_word_wstrb_next <= 0;
        ram1_word_burst_len <= 0; ram1_word_burst_wr_len <= 0;
        sdram_accepted_r <= 0; sdram_cmd_forwarded <= 0;
    end else begin
        ram1_word_rd <= 0; ram1_word_wr <= 0;
        ram1_word_burst_len <= 4'd0; ram1_word_burst_wr_len <= 4'd0;
        sdram_accepted_r <= 0;
        if (!sdram_slave_rd && !sdram_slave_wr)
            sdram_cmd_forwarded <= 0;
        if (!ram1_word_busy && !sdram_cmd_forwarded &&
            (sdram_slave_rd || sdram_slave_wr)) begin
            ram1_word_rd <= sdram_slave_rd;
            ram1_word_wr <= sdram_slave_wr;
            ram1_word_addr <= sdram_slave_addr;
            ram1_word_data <= sdram_slave_wdata;
            ram1_word_wstrb <= sdram_slave_wstrb;
            ram1_word_data_next <= sdram_slave_preload_wdata;
            ram1_word_wstrb_next <= sdram_slave_preload_wstrb;
            ram1_word_burst_len <= sdram_slave_burst_len;
            ram1_word_burst_wr_len <= sdram_slave_burst_wr_len;
            sdram_accepted_r <= 1;
            sdram_cmd_forwarded <= 1;
        end
    end
end

// ============================================================
// Arbiter — M0 = gpu_core (rd -> AR, wr -> AW/W).  No M1/M2/M3 aggressors:
// the scanout burst_rd (highest io_sdram priority) is the contention source
// here, exactly as on real MiSTer where the LCD line fetch outranks the GPU.
// ============================================================
axi_sdram_arbiter sdram_arb (
    .clk(clk), .reset_n(reset_n),
    .m0_arvalid(gpu_rd_arvalid), .m0_arready(gpu_rd_arready),
    .m0_araddr(gpu_rd_araddr),   .m0_arlen(gpu_rd_arlen),
    .m0_rvalid(gpu_rd_rvalid),   .m0_rdata(gpu_rd_rdata),
    .m0_rresp(),                 .m0_rlast(gpu_rd_rlast),
    .m0_awvalid(gpu_wr_awvalid), .m0_awready(gpu_wr_awready),
    .m0_awaddr(gpu_wr_awaddr),   .m0_awlen(gpu_wr_awlen),
    .m0_wvalid(gpu_wr_wvalid),   .m0_wready(gpu_wr_wready),
    .m0_wdata(gpu_wr_wdata),     .m0_wstrb(gpu_wr_wstrb),
    .m0_wlast(gpu_wr_wlast),
    .m0_bvalid(gpu_wr_bvalid),   .m0_bresp(),

    .m1_arvalid(1'b0), .m1_arready(),
    .m1_araddr(32'b0), .m1_arlen(8'b0),
    .m1_rvalid(),      .m1_rdata(),
    .m1_rresp(),       .m1_rlast(),
    .m1_rready(1'b1),
    .m1_awvalid(1'b0), .m1_awready(),
    .m1_awaddr(32'b0), .m1_awlen(8'b0),
    .m1_wvalid(1'b0),  .m1_wready(),
    .m1_wdata(32'b0),  .m1_wstrb(4'b0),
    .m1_wlast(1'b0),
    .m1_bvalid(),      .m1_bresp(),

    .m2_arvalid(1'b0), .m2_arready(),
    .m2_araddr(32'b0), .m2_arlen(8'b0),
    .m2_rvalid(),      .m2_rdata(),
    .m2_rresp(),       .m2_rlast(),
    .m2_rready(1'b1),
    .m2_awvalid(1'b0), .m2_awready(),
    .m2_awaddr(32'b0), .m2_awlen(8'b0),
    .m2_wvalid(1'b0),  .m2_wready(),
    .m2_wdata(32'b0),  .m2_wstrb(4'b0),
    .m2_wlast(1'b0),
    .m2_bvalid(),      .m2_bresp(),

    .m3_arvalid(1'b0), .m3_arready(),
    .m3_araddr(32'b0), .m3_arlen(8'b0),
    .m3_rvalid(),      .m3_rdata(),
    .m3_rresp(),       .m3_rlast(),
    .m3_rready(1'b1),

    .s_arvalid(arb_s_arvalid), .s_arready(arb_s_arready),
    .s_araddr(arb_s_araddr),   .s_arlen(arb_s_arlen),
    .s_rvalid(arb_s_rvalid),   .s_rready(arb_s_rready),
    .s_rdata(arb_s_rdata),
    .s_rresp(arb_s_rresp),     .s_rlast(arb_s_rlast),
    .s_awvalid(arb_s_awvalid), .s_awready(arb_s_awready),
    .s_awaddr(arb_s_awaddr),   .s_awlen(arb_s_awlen),
    .s_wvalid(arb_s_wvalid),   .s_wready(arb_s_wready),
    .s_wdata(arb_s_wdata),     .s_wstrb(arb_s_wstrb),
    .s_wlast(arb_s_wlast),
    .s_bvalid(arb_s_bvalid),   .s_bresp(arb_s_bresp),
    .s_wcont(arb_s_wcont),
    .dbg_arb_state(), .dbg_cpu_pending(), .dbg_grant()
);

axi_sdram_slave #(
    .SERIALIZE_WRITE_BURSTS(1'b0),
    .MAX_NATIVE_WRITE_BURST_LEN(8'd15)
) slave (
    .clk(clk), .reset_n(reset_n),
    .s_axi_arvalid(arb_s_arvalid), .s_axi_arready(arb_s_arready),
    .s_axi_araddr(arb_s_araddr),   .s_axi_arlen(arb_s_arlen),
    .s_axi_rvalid(arb_s_rvalid),   .s_axi_rready(arb_s_rready),
    .s_axi_rdata(arb_s_rdata),     .s_axi_rresp(arb_s_rresp), .s_axi_rlast(arb_s_rlast),
    .s_axi_awvalid(arb_s_awvalid), .s_axi_awready(arb_s_awready),
    .s_axi_awaddr(arb_s_awaddr),   .s_axi_awlen(arb_s_awlen),
    .s_axi_wvalid(arb_s_wvalid),   .s_axi_wready(arb_s_wready),
    .s_axi_wdata(arb_s_wdata),     .s_axi_wstrb(arb_s_wstrb), .s_axi_wlast(arb_s_wlast),
    .s_axi_bvalid(arb_s_bvalid),   .s_axi_bready(1'b1),       .s_axi_bresp(arb_s_bresp),
    .s_axi_wcont(arb_s_wcont),
    .sdram_rd(sdram_slave_rd), .sdram_wr(sdram_slave_wr),
    .sdram_addr(sdram_slave_addr), .sdram_wdata(sdram_slave_wdata), .sdram_wstrb(sdram_slave_wstrb),
    .sdram_burst_len(sdram_slave_burst_len), .sdram_burst_wr_len(sdram_slave_burst_wr_len),
    .sdram_rdata(ram1_word_q), .sdram_busy(ram1_word_busy),
    .sdram_accepted(sdram_accepted_r), .sdram_rdata_valid(ram1_word_q_valid),
    .sdram_wr_data_next(sdram_slave_wr_data_next),
    .sdram_wr_done(sdram_slave_wr_done),
    .sdram_next_wdata(sdram_slave_next_wdata),
    .sdram_next_wstrb(sdram_slave_next_wstrb),
    .sdram_preload_wdata(sdram_slave_preload_wdata),
    .sdram_preload_wstrb(sdram_slave_preload_wstrb)
);

// ============================================================
// SCANOUT — REAL video_CRT_scanout_indexed_BRAM, LCD line fetch only.
// burst_rd goes straight into io_sdram's burst_rd port (hard-real-time, top
// io_sdram ST_IDLE priority above the GPU word ops).  fb_base_addr =
// fb_display_addr from the swap SM, so the scanout reads whatever buffer is
// CURRENTLY DISPLAYED from the SHARED SDRAM.
// ============================================================
wire        scan_burst_rd;
wire [24:0] scan_burst_addr;
wire [10:0] scan_burst_len;
wire        scan_burst_32bit;
wire [31:0] scan_burst_data;
wire        scan_burst_data_valid;
wire        scan_burst_data_done;
wire [23:0] lcd_pixel_color;

assign scanout_burst_addr_o        = scan_burst_addr;
assign scanout_burst_rd_o          = scan_burst_rd;
assign scanout_burst_data_valid_o  = scan_burst_data_valid;
assign scanout_burst_data_o        = scan_burst_data;

video_CRT_scanout_indexed_BRAM #(
    .HAS_ANALOG_RASTER(0)   // LCD-only: MiSTer prunes the analog raster cone
) scanout (
    .clk_video(clk_vid),
    .reset_n(reset_n),
    .x_count(x_count),
    .y_count(y_count),
    .line_start(line_start),
    .pixel_color(lcd_pixel_color),

    .clk_analog(clk_vid),       // MiSTer: clk_analog == clk_vid
    .reset_analog_n(reset_n),
    .analog_ce_pix(analog_ce_pix),
    .analog_scanlines(2'd0),
    .analog_timing(2'd1),       // 480p (slaved to LCD); analog raster pruned anyway
    .analog_pixel_clk(), .analog_pixel_color(),
    .analog_hblank(), .analog_vblank(), .analog_hsync(), .analog_vsync(),

    .fb_base_addr(fb_display_addr),
    .color_mode(color_mode_in),
    .fb_width(fb_w),
    .fb_height(fb_h),
    .fb_stride(fb_strideb),
    .out_width(out_w),
    .out_height(crt_v_active),

    .analog_fb_base_addr(fb_display_addr),
    .analog_color_mode(color_mode_in),
    .analog_fb_width(fb_w),
    .analog_fb_height(fb_h),
    .analog_fb_stride(fb_strideb),
    .analog_out_width(out_w),
    .analog_out_height(crt_v_active),

    .clk_palette(clk),
    .reset_palette_n(reset_n),

    .clk_sdram(clk),
    .burst_rd(scan_burst_rd),
    .burst_addr(scan_burst_addr),
    .burst_len(scan_burst_len),
    .burst_32bit(scan_burst_32bit),
    .burst_data(scan_burst_data),
    .burst_data_valid(scan_burst_data_valid),
    .burst_data_done(scan_burst_data_done),

    .pal_wr(1'b0), .pal_addr(8'd0), .pal_data(24'd0),
    .pal_commit(1'b0), .pal_busy()
);

// ============================================================
// io_sdram (MiSTer controller; split-DQ test variant).  burst_rd = REAL
// scanout fetch; word ports = GPU FB writes via the slave.
// ============================================================
io_sdram #(
    .BANK_ROW_TRACK(BANK_ROW_TRACK)
) sdram_ctrl (
    .controller_clk(clk), .chip_clk(clk), .clk_90(clk), .reset_n(reset_n),
    .phy_cke(phy_cke), .phy_clk(phy_clk),
    .phy_cas(phy_cas), .phy_ras(phy_ras), .phy_we(phy_we),
    .phy_ba(phy_ba), .phy_a(phy_a),
    .phy_dq_in(dq_to_ctrl),
    .phy_dq_out_port(ctrl_dq_out),
    .phy_dq_oe_port(ctrl_dq_oe),
    .phy_dqm(phy_dqm),
    .burst_rd(scan_burst_rd), .burst_addr(scan_burst_addr),
    .burst_len(scan_burst_len), .burst_32bit(scan_burst_32bit),
    .burst_data(scan_burst_data), .burst_data_valid(scan_burst_data_valid),
    .burst_data_done(scan_burst_data_done),
    .burstwr(1'b0), .burstwr_addr(25'b0), .burstwr_ready(),
    .burstwr_strobe(1'b0), .burstwr_data(16'b0), .burstwr_done(1'b0),
    .word_rd(ram1_word_rd), .word_wr(ram1_word_wr),
    .word_addr(ram1_word_addr), .word_data(ram1_word_data), .word_wstrb(ram1_word_wstrb),
    .word_data_next(ram1_word_data_next), .word_wstrb_next(ram1_word_wstrb_next),
    .word_burst_len(ram1_word_burst_len), .word_burst_wr_len(ram1_word_burst_wr_len),
    .word_q(ram1_word_q), .word_busy(ram1_word_busy), .word_q_valid(ram1_word_q_valid),
    .word_wr_data_next(sdram_slave_wr_data_next),
    .word_wr_done(sdram_slave_wr_done),
    .burst_wr_direct_data(sdram_slave_next_wdata),
    .burst_wr_direct_strb(sdram_slave_next_wstrb),
    .dbg_io()
);

// ============================================================
// Shared SDRAM model (full 10-bit column) — holds all 3 FBs + ring + tex.
// ============================================================
sdram_model_full sdram_chip (
    .clk(phy_clk), .cke(phy_cke), .cs_n(1'b0),
    .ras_n(phy_ras), .cas_n(phy_cas), .we_n(phy_we),
    .ba(phy_ba), .a(phy_a),
    .dq_in(ctrl_dq_out),
    .dq_out(model_dq_out), .dq_oe(model_dq_oe),
    .dqm(phy_dqm),
    .bd_we(bd_we), .bd_word_addr(bd_addr), .bd_wdata(bd_wdata),
    .bd_rd_word_addr(bd_rd_addr), .bd_rd_data(bd_rd_data)
);

wire _unused = &{1'b0, lcd_pixel_color, ctrl_dq_oe, phy_clk, phy_cke, 1'b0};

endmodule

`default_nettype wire
