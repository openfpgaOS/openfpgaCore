...
typedef struct {
    ...
    uint8_t         name_len;
    /* name[name_len] follows; declared with a 1-element size instead
     * of a true flexible array so this struct can be embedded in
     * l9660_vdesc_primary (C++ rejects FAMs not at end of enclosing
     * struct). */
    char            name[1];
} l9660_dirent;
...
/*! Decode a directory-entry name for presentation/path matching.
 * ISO9660 version suffixes (";1") are stripped.  Joliet names are decoded
 * from UCS-2BE to UTF-8.  Returns the number of output bytes, excluding NUL. */
