...
## Toolchain (installed locally — check `which` before claiming a tool is missing)

- **Quartus Prime Lite 25.1** — `/home/alberto/altera_lite/25.1std/quartus/bin` (the pocket Makefile prepends this to `PATH`)
- **sbt 1.x** — regenerates the VexiiRiscv netlist (runs in a Docker container **by default** now; host sbt/JDK optional — see *CPU netlist*)
- **riscv64-elf-gcc** — firmware/OS + apps (RV32IMAFC, musl static)
- **Verilator 5.x** — RTL test suite
- **Docker** — isolated / parallel Quartus builds (image `openfpgaos-quartus`; Quartus bind-mounted, not baked in)
...
- **One netlist per variant**, generated separately: `VexiiRiscv_os25.v`, `VexiiRiscv_os30.v`, `VexiiRiscv_mister.v`. For pocket, `vexii_active.v` picks os25 vs os30 via the `VEXII_CPU_OS30` macro (set only in os30's `VARIANT_DEFS`). mister's qsf includes its netlist directly.
